...
# Script to repair UUID migrations using the full filename format (without .sql)
# Supabase stores migrations by their full filename in the migration history

set -e

echo "Repairing UUID migrations using full filename format..."
...
echo "Found $TOTAL UUID migrations to repair"
...
if [ $FAILED -eq 0 ]; then
  ...
  echo ""
  echo "Some migrations failed. These may already be marked correctly or don't exist."
