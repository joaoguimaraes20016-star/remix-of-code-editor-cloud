 ...
 * These look like actual form elements, not abstract icons
 */
...
export function VideoQuestionMockup() {
  return (
    <div className="w-full flex flex-col gap-1.5">
      {/* Title bar */}
      <div className="w-16 h-1.5 bg-blue-400 rounded mx-auto" />
      ...
      {/* Video thumbnail */}
      <div className="mx-2 aspect-video bg-gradient-to-br from-slate-700 to-slate-900 rounded-lg flex items-center justify-center relative overflow-hidden">
        {/* Person silhouette */}
        <div className="w-6 h-6 bg-slate-600 rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
        {/* Play button overlay */}
        <div className="w-6 h-6 bg-white/90 rounded-full flex items-center justify-center z-10">
          <Play size={10} className="text-slate-800 ml-0.5" fill="currentColor" />
        </div>
      </div>
    </div>
  );
