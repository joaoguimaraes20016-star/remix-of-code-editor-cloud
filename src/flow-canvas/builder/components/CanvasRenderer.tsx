...
const SortableElementRenderer = React.forwardRef<HTMLDivElement, SortableElementRendererProps>(({ 
  ...
  const renderElement = () => {
    ...
    switch (element.type) {
      ...
      // Form Group element - multi-field form container
      case 'form-group': {
        const formFields = (element.props?.fields as Array<{
          id: string;
          type: string;
          placeholder: string;
          required?: boolean;
        ...
        const formLayout = (element.props?.layout as string) || 'vertical';
        const formGap = (element.props?.gap as number) || 12;
        ...
        return (
          <div ref={combinedRef} style={style} className={cn(baseClasses, 'w-full relative')} {...stateHandlers}>
            ...
            <div 
              className={cn(
                'flex w-full',
                formLayout === 'vertical' ? 'flex-col' : 'flex-row flex-wrap'
              )}
              style={{ gap: formGap }}
              onClick={(e) => { e.stopPropagation(); onSelect(); }}
            >
              {formFields.length > 0 ? formFields.map((field) => (
                <input
                  ...
                  className={cn(
                    "w-full px-4 py-3 border rounded-xl bg-transparent transition-colors",
                    formLayout === 'horizontal' && "flex-1 min-w-[200px]"
                  )}
                  style={{
                    borderColor: isDarkTheme ? '#374151' : '#e5e7eb',
                ...
                <div className={cn(
                  "text-sm text-center py-4 w-full",
                  isDarkTheme ? "text-gray-500" : "text-gray-400"
                )}>
                  Add form fields in the inspector
                </div>
              )}
            </div>
          </div>
      ...
      // Feature List element - list with icons
      case 'feature-list': {
        const featureItems = (element.props?.items as Array<{
          id: string;
          text: string;
          icon?: string;
        }>) || [];
        const featureIconColor = (element.props?.iconColor as string) || '#22C55E';
        const featureLayout = (element.props?.layout as string) || 'vertical';
        const featureGap = (element.props?.gap as number) || 8;
        ...
        return (
          <div ref={combinedRef} style={style} className={cn(baseClasses, 'w-full relative')} {...stateHandlers}>
            ...
            <div 
              className={cn(
                'flex',
                featureLayout === 'vertical' ? 'flex-col' : 'flex-row flex-wrap'
              )}
              style={{ gap: featureGap }}
              onClick={(e) => { e.stopPropagation(); onSelect(); }}
            >
              {featureItems.length > 0 ? featureItems.map((item) => (
                <div key={item.id} className="flex items-center gap-3">
                  <div 
                    ...
                    style={{ backgroundColor: `${featureIconColor}20` }}
                  >
                    <Check className="w-3 h-3" style={{ color: featureIconColor }} />
                  </div>
      ...
      // Testimonial element - quote with author info
      case 'testimonial': {
        const testimonialAuthor = (element.props?.author as string) || 'John Doe';
        const testimonialRole = (element.props?.role as string) || 'CEO';
        const testimonialCompany = (element.props?.company as string) || '';
        const testimonialAvatar = (element.props?.avatar as string) || '';
        const testimonialRating = (element.props?.rating as number) || 5;
        const testimonialQuote = element.content || 'Great product!';
                ...
                <div className="flex items-center gap-0.5 mb-3">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <svg 
                      ...
                      className="w-4 h-4" 
                      viewBox="0 0 20 20" 
                      fill={star <= testimonialRating ? '#FACC15' : '#E5E7EB'}
                    >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={cn(
                    ...
                    {testimonialRole}{testimonialCompany && `, ${testimonialCompany}`}
                  </p>
                </div>
              </div>
            </div>
      ...
      // Single Choice / Multiple Choice elements
      case 'single-choice':
      case 'multiple-choice': {
        const choiceOptions = (element.props?.options as Array<{
          id?: string;
          label: string;
          value: string;
          imageUrl?: string;
        ...
        const choiceLayout = (element.props?.layout as string) || 'vertical';
        const choiceName = (element.props?.name as string) || element.id;
        const isMultipleChoice = element.type === 'multiple-choice';
        ...
        return (
          <div ref={combinedRef} style={style} className={cn(baseClasses, 'w-full relative')} {...stateHandlers}>
            ...
            {!readOnly && (
              <UnifiedElementToolbar
                elementId={element.id}
                elementType={element.type as 'single-choice' | 'multiple-choice'}
                elementLabel={isMultipleChoice ? 'Multiple Choice' : 'Single Choice'}
                isSelected={isSelected}
                targetRef={wrapperRef}
                deviceMode={deviceMode}
                dragHandleProps={{ attributes, listeners }}
            ...
            <div 
              className={cn(
                'flex gap-2',
                choiceLayout === 'vertical' ? 'flex-col' : 'flex-row flex-wrap',
                choiceLayout === 'grid' && 'grid grid-cols-2'
              )}
              onClick={(e) => { e.stopPropagation(); onSelect(); }}
            >
              {choiceOptions.map((option, idx) => {
                const optionId = option.id || `${choiceName}-${idx}`;
                ...
                return (
                  <label
                    ...
                    className={cn(
                      ...
                      hasImage && "flex-col p-0 overflow-hidden",
                      isDarkTheme 
